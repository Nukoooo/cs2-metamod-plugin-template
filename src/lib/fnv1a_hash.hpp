...
struct fnv1a
{
    ...
    static constexpr Type hash_container(T s, const std::size_t l, L stopLen = nullptr, Type hash = fnv1a_traits<Bits>::Offset)
    {
        ...
        for (; j < l; j++)
        {
            ...
            if constexpr (stop != 0)
            {
                if (byte == stop)
                {
                    if constexpr (!std::is_same_v<L, decltype(nullptr)>)
                    {
                        *stopLen = j + 1;
                    }
                    break;
    ...
    static constexpr Type hash(const C* s, const std::size_t l, L stopLen = nullptr, Type hash = fnv1a_traits<Bits>::Offset)
    {
    ...
    {
        return hash(s, __builtin_strlen(s));
    }
    ...
    class _lowercase_container
    {
      public:
        ...
        constexpr auto operator[](std::size_t index) const
        {
            ...
            static_assert(std::is_integral_v<decltype(c)>);
            static_assert(sizeof(c) == 1);
